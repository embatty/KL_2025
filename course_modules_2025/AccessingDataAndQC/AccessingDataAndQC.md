...
Let's assess the sequence data quality of an isolate of *Escherichia coli* (ERR5386320). This data came from the European Nucleotide Archive, a data archive which contains public sequencing data from thousands of bacterial sequencing projects. ERR5386320 is the accession number which allows you to find this data in the public archives.
...
The data for this exercise has already been downloaded for you, but if you want to see the commands you would use to download the data they are presented here for reference - click on the small arrow to open the details.

<details>
    <summary>File download from ENA</summary>
Download the sequence reads (fastq files) for analysis by entering the following commands one after the other:

```
curl --output ERR5386320_1.fastq.gz ftp://ftp.sra.ebi.ac.uk/vol1/fastq/ERR538/000/ERR5386320/ERR5386320_1.fastq.gz
curl --output ERR5386320_2.fastq.gz ftp://ftp.sra.ebi.ac.uk/vol1/fastq/ERR538/000/ERR5386320/ERR5386320_2.fastq.gz
```

Or you can use the script downloaded by clicking on the "Download All" link on the ENA web page.
</details>

## Running the FastQC tool<a name="runfastqc"></a>

To run `fastqc`, we need to load the conda environment for this module:

`conda activate AccessingDataAndQC`

Run the following command to run the fastqc tool on both the read files:

`fastqc data/ERR5386320_1.fastq.gz data/ERR5386320_2.fastq.gz`

If the command does not run, check that you have not made an error in the names of the files, and also make sure that the `fastq.gz` files are in the `data` subdirectory of the directory where you are running the command.
...
Now run the fastqc tool on the sequence reads of an isolate of _Pseudomonas aeruginosa_. The ENA accession number is ERR5386380. The files are located within the folder `AccessingDataAndQc/data`. Analyse the graphs generated by the tool and answer the following questions:
...
`fastp --in1 data/ERR5386380_1.fastq.gz --in2 data/ERR5386380_2.fastq.gz --out1 ERR5386380_1.trimmed.fastq.gz --out2 ERR5386380_2.trimmed.fastq.gz --length_required 40 --cut_front --cut_tail --cut_mean_quality 25`
