...
Now open the terminal and change your working directory to the directory with the files for this practical, which is a folder called AccessingDataAndQC. If you put the data in your `home/data` folder, you can navigate to this directory:

`cd /home/data/AccessingDataAndQC`
...
If the command does not run, check that you have not made an error in the names of the files, and also make sure that the `fastq.gz` files are in the directory where you are running the command.
...
Now run the fastqc tool on the sequence reads of an isolate of _Pseudomonas aeruginosa_. The ENA accession number is ERR5386380. The files are located within the folder `AccessingDataAndQc`. Analyse the graphs generated by the tool and answer the following questions:
