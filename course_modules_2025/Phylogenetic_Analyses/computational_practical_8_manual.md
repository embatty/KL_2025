...
In the previous session, we ran **Snippy** to map the short Illumina reads and **call SNPs** for an individual *Klebsiella pneumoniae* isolate from a suspected hospital outbreak. Now will attempt to construct a molecular phylogeny from this so-called *SNP alignment*.
...
cd cp8
```
We have created the whole-genome sequence alignment by concatenating the Snippy consensus sequence of all 14 K. pneumoniae ST78 isolates. This file is called `Kpn_ST78.cpe058.fas`.
...
```
iqtree2 -s Kpn_ST78.cpe058.strain_ids.fas -T 1 --mem 4G --ufboot 1000 --prefix Kpn_ST78.cpe058_iqtree -wbtl
```
...
```
run_gubbins.py Kpn_ST78.cpe058.strain_ids.fas --prefix Kpn_ST78.cpe058.gubbins --use-time-stamp --threads 1 --first-tree-builder fasttree --tree-builder raxml --outgroup Germany_2019_Kpn_ST78
```
